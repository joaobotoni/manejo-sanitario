...
public interface ItemDao {
    ...
    @Query("SELECT * FROM xgp_item WHERE id_tipo_item = :idTipoItem order by descricao")
    List<Item> findByTipoItem(int idTipoItem);

    @Query("SELECT * FROM xgp_item WHERE id_tipo_item in (2, 6) order by descricao")
    List<Item> findByMaqVeic();
